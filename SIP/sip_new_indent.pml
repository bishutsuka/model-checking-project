...
proctype agent(byte i) {
	...
	idle:
	    ...
	    :: atomic{proxy2agent[i]?sessionProgress ->
	    	if
		    :: atomic{ cancelsent[i] == 1 -> goto waiting_for_canceled;}
		    :: do 
	...
	inviting:
		if
		:: atomic{agent2proxy[i]!invite; invitesent[i] = 1;
		   do
	...
	waiting_for_trying:
		...
		:: atomic{agent2agent[i]?ack -> goto media_session_server;}	
		:: atomic{agent2agent[i]?byeOk -> goto waiting_for_trying;}
		:: atomic{proxy2agent[i]?canceling -> goto waiting_for_canceled;}
		:: atomic{proxy2agent[i]?trying ->
			if
		    :: atomic{cancelsent[i] == 1 -> goto waiting_for_canceled;}
	...
	waiting_for_session_progress:
		...
		:: atomic{agent2agent[i]?byeOk -> goto waiting_for_session_progress;}
		:: atomic{proxy2agent[i]?canceling -> goto waiting_for_canceled;}
		:: atomic{proxy2agent[i]?sessionProgress ->
			if
		    :: atomic{ cancelsent[i] == 1 -> goto waiting_for_canceled;}
	...
	waiting_for_ok:
		...
		:: atomic{agent2agent[i]?ack -> goto media_session_server;}	
		:: atomic{agent2agent[i]?byeOk -> goto waiting_for_ok;}
	...
	waiting_for_bye:
		if
		:: atomic{agent2agent[i]?byeOk -> goto waiting_for_bye;}
		:: atomic{agent2agent[i]?ack -> goto media_session_server;}
		:: atomic{agent2agent[i]?bye -> agent2agent[j]!byeOk; invitesent[i] = 0; goto idle;}				/* session terminated */
		fi;
    waiting_for_canceled:
	    ...
	    :: atomic{agent2agent[i]?byeOk -> goto waiting_for_canceled;}
	    :: atomic{agent2agent[i]?ack -> goto media_session_server;}
	...
	waiting_for_session_termination:
	    ...
	    :: atomic{ agent2agent[i]?bye -> agent2agent[j]!byeOk; invitesent[i] = 0; goto waiting_for_session_termination; }
		:: atomic{ agent2agent[i]?byeOk -> goto idle;}							/* session terminated */
		fi;
}
...
ltl p4 { [](agent[0]@inviting -> (!agent[0]@inviting U agent[0]@idle)) && [](agent[1]@inviting -> (!agent[1]@inviting U agent[1]@idle)) }
